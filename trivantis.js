...
{		
	...
	if(is.isMobile.any()){
		if(is.isMobile.iOS() && isInFrame(getDisplayWindow())) {
...
function createdInteractDiv(cb)
{
	var nDiv = getDisplayDocument().createElement('div');
	...
	{
		cb();
		triv$('#initClickDiv', getDisplayDocument()).remove();
	};
	var pTag = getDisplayDocument().createElement('span');
